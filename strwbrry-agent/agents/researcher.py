"""
Researcher agent definition and worker entry point
"""
...
llm=gemini(
        model=os.getenv("RESEARCHER_MODEL", "gemini-2.5-flash"),
        temperature=0.2
    ),
    tools=TOOLS,
is_coordinator=False,  # Researcher is a specialist, not a coordinator
max_retries=3,
    max_iterations=15,
max_tool_calls=4,
timeout=600,
...
async def main():
    ...
    print(f"Starting researcher worker...")
    await runner.start()
